...
func TryProblemDampener(arr []int) bool {
	for i := 0; i < len(arr); i++ {
		// if i directly do append(arr[:i], arr[i+1:]...), it will modify the underlying array
		subarr := make([]int, len(arr)-1)
		copy(subarr, arr[:i])       // copy elements before the removed index
		copy(subarr[i:], arr[i+1:]) // copy elements after the removed index

		valid, err := IsValidReport(subarr)
		if err != nil {
			continue
		}
...
func main() {
	...
	for _, line := range lines {
		...
		if valid || TryProblemDampener(ints) {
			total += 1
			fmt.Printf("valid: %v\n", ints)
		}
	}
